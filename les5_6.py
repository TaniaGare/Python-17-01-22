# ###########################################################################
# формировать (не программно) текстовый файл. В нём каждая строка должна 
...
with open("les5_6.txt", "r", encoding = 'utf-8') as my_file:
    my_dict = {}
    ...
    for line in my_read:
        if len(line):
            ...
            for hours in subject[1:]:
                if len(hours) > 1:
                   total_hours += int(hours.split('(')[0])
            my_dict[subject[0]] = total_hours
print(f" {my_dict} ")
