...
#  методы должны применяться только к клеткам и выполнять увеличение, уменьшение, у
#  множение и целочисленное (с округлением до целого) деление клеток, соответственно.
...
class Cell:
    ...
    def __sub__(self, other):
        return Cell(self.numb - other.numb) if self.numb - other.numb > 0 else "вычитание невозможно"
    ...
    def make_order(self, row):
        result = ''
        for i in range(int(self.numb / row)):
            result += '*' * row + '\n'
        result += '*' * (self.numb % row) + '\n'
        return result
